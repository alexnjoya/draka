...
class LoginScreen(tb.Frame):
    def __init__(self, parent, controller):
        ...
        tb.Label(card, text='Login', font=('Segoe UI', 24, 'bold')).pack(pady=(30, 10))

        form = tb.Frame(card, bootstyle="light")
        form.pack(pady=10)
        tb.Label(form, text='Email', font=('Segoe UI', 12)).grid(row=0, column=0, sticky='e', padx=10, pady=10)
        self.email_entry = tb.Entry(form, width=28, bootstyle=PRIMARY)
        self.email_entry.grid(row=0, column=1, padx=10, pady=10)
        tb.Label(form, text='Password', font=('Segoe UI', 12)).grid(row=1, column=0, sticky='e', padx=10, pady=10)
        self.password_entry = tb.Entry(form, show='*', width=28, bootstyle=PRIMARY)
        self.password_entry.grid(row=1, column=1, padx=10, pady=10)

        btn_frame = tb.Frame(card, bootstyle="light")
        btn_frame.pack(pady=20)
        tb.Button(btn_frame, text='Login', bootstyle=SUCCESS, width=14, command=self.login).grid(row=0, column=0, padx=8)
        tb.Button(btn_frame, text='Sign Up', bootstyle=SECONDARY, width=14, command=lambda: controller.show_frame('SignupScreen')).grid(row=0, column=1, padx=8)
